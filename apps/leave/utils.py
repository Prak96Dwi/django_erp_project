...
def web_developers_list(all_members):
    ...
    """
    return [
        member for member in all_members if member.designation == settings.WEB_DEVELOPER
    ]
...
def directors_list(all_members):
    ...
    """
    return [member for member in all_members if member.designation == settings.DIRECTOR]
...
def tech_leaders_list(all_members):
    ...
    """
    return [
        member for member in all_members if member.designation == settings.TECH_LEADER
    ]
...
def send_notification_to_tech_lead_and_hiring_manager(
    request,
    tech_lead,
    message,
    message_description,
    ...
    notify.send(
        ...
        action_object=hiring_manager
    )


def mail_for_leave(message_description, hiring_manager_email, tech_lead_email):
    subject = 'Leave Request'
    message = message_description
    email_from = settings.EMAIL_HOST_USER
    recipient_list = [hiring_manager_email, tech_lead_email]
